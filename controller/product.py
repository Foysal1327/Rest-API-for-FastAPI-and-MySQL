...
from repository.product_request import ProductRequest, ProductUpdateRequest
...
async def read_all_products(created_by: str, page_size: int, page: int):
    session = database.get_db_session(engine)
    data = session.query(Product).filter(
        and_(Product.created_by == created_by, Product.deleted == False)
        ).order_by(desc(Product.created_at)).limit(page_size).offset((page-1)*page_size).all()
